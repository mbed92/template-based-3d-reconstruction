...
arma::mat FernKeypointMatcher3D2D::MatchImages3D2D( IplImage* inputImageRGB, vector<cv::Point2f> &kp )
{
	...
	for(int i = 0; i < detector->number_of_model_points; i++)
	{
		...
		{
			this->matches3D2D(nMatches, span(0,5)) = this->bary3DRefKeypoints.row(i);

			this->matches3D2D(nMatches, 6) = detector->model_points[i].potential_correspondent->fr_u();
			this->matches3D2D(nMatches, 7) = detector->model_points[i].potential_correspondent->fr_v();

			// ID of the 3D point of the match
			this->matches3D2D(nMatches, 8) = i;
			nMatches++;
		}
	}

	if (nMatches > 0)
		return this->matches3D2D.rows(0, nMatches-1);
	else
