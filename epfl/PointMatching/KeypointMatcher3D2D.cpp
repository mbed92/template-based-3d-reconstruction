					...
					, camCamera		( camCamera		)
{
    ...
    cvCopy(referenceImgRGB, this->referenceImgRGB);
    ...
    cvCopy(this->referenceImgGray, this->templateImageGray);
...
arma::mat KeypointMatcher3D2D::MatchImages3D2D( IplImage* inputImageRGB )
{
    ...
    Visualization::DrawAQuadrangle(cv::cvarrToMat(referenceImgRGB), topLeft, topRight, bottomRight, bottomLeft, TPL_COLOR);

    Visualization::VisualizeMatches(cv::cvarrToMat(this->referenceImgRGB), cv::cvarrToMat(inputImageRGB), matches2D2D, 10);
	// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	for (int i = 0; i < nMatches; i++)
	{
		...
		if (isIntersect)
		{
            // from 0 to 5: ABC (vertices of triangle) and BARRY (coords in ABC)
            this->matches3D2D(count, span(0, 5)) = intersectPoint;

            // from 6 to 7: u and v on input image
...
bool KeypointMatcher3D2D::find3DPointOnMesh(const cv::Point2d& refPoint, rowvec& intersectionPoint) const
{
	...
	for (int i = 0; i < nFacets; i++)
	{
		...
		if (bary(0) >= 0 && bary(1) >= 0 && bary(2) >= 0)
		{
			...
			{
				minDepth = depth;
                intersectionPoint << aFacet(0) << aFacet(1) << aFacet(2) << bary(0) << bary(1) << bary(2) << endr;
				found = true;
			}
		}
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
